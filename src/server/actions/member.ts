...
export async function inviteMember(
  ...
  return withErrorHandling<{ added: boolean; inviteUrl?: string }>(async () => {
    ...
    if (!canManageRole(membership!.role, data.role)) {
      return fail(`You cannot invite someone as ${ROLE_LABEL[data.role]}.`);
    }
    ...
    if (existingUser) {
      ...
      if (already) return fail("They are already a member.");
      ...
      await Promise.all([
        logActivity({
          ...
          actorId: user.id,
          type: "MEMBER_JOINED",
          message: `${user.name} added ${existingUser.name} to the workspace`,
        }),
        notify({
          ...
          body: `Role: ${ROLE_LABEL[data.role]}`,
    ...
    void sendEmail({
      to: email,
      subject: `${user.name} invited you to join ${workspace.name} on Kanovra`,
...
export async function updateMemberRole(input: unknown): Promise<ActionResult> {
  return withErrorHandling(async () => {
    ...
    if (!target || target.workspaceId !== data.workspaceId) return fail("Member not found.");
    if (target.userId === user.id) return fail("You cannot change your own role.");
    if (!canManageRole(actor!.role, target.role) || !canManageRole(actor!.role, data.role)) {
      return fail("You do not have permission to change that role.");
    }
    if (target.workspace.ownerId === target.userId) {
      return fail("The owner's role cannot be changed.");
    }
    ...
    await Promise.all([
      logActivity({
        ...
        message: `${user.name} changed ${target.user.name}'s role to ${ROLE_LABEL[data.role]}`,
      }),
      notify({
        ...
        body: `Role new: ${ROLE_LABEL[data.role]}`,
...
export async function removeMember(memberId: string): Promise<ActionResult> {
  return withErrorHandling(async () => {
    ...
    if (!can(actor?.role, "workspace:manage_members")) throw new ForbiddenError();
    if (target.workspace.ownerId === target.userId) return fail("The owner cannot be removed.");
    if (!canManageRole(actor!.role, target.role)) return fail("You do not have permission to remove that member.");
    ...
    await logActivity({
      ...
      actorId: user.id,
      type: "MEMBER_REMOVED",
      message: `${user.name} removed ${target.user.name} from the workspace`,
    });
...
export async function transferOwnership(
  ...
  return withErrorHandling(async () => {
    ...
    if (workspace.ownerId !== user.id) return fail("Only the owner can transfer ownership.");

    const target = await getMembership(newOwnerUserId, workspaceId);
    if (!target) return fail("The recipient must already be a workspace member.");
